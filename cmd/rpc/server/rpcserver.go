...
func main() {
	file, err := os.OpenFile("rpc-server-log.txt", os.O_CREATE|os.O_APPEND|os.O_WRONLY, 0666)
	if err != nil {
		log.Fatal(err)
	}
	defer func() {
...
func handleConn(conn net.Conn) {
	...
	case rpc.Upload:
		...
		err = ioutil.WriteFile(rpc.ServerFilesPath + options, bytes, 0666)
		if err != nil {
	...
	case rpc.Download:
		...
		file, err := os.Open(rpc.ServerFilesPath + options)
		if err != nil {
	...
	case rpc.FileList:
		...
		fileName := rpc.GetFileList(rpc.ServerFilesPath)
