...
func downloadFromServer(conn net.Conn, fileName string) {
	...
	err = ioutil.WriteFile(rpc.ClientFilesPath + fileName, bytes, 0666)
	if err != nil {
...
func uploadInServer(conn net.Conn, fileName string) {
	...
	file, err := os.Open(rpc.ClientFilesPath + options)
	writer := bufio.NewWriter(conn)
	if err != nil {
...
func listFile(conn net.Conn) {
	...
	fmt.Println("Список файлов доступных для скачивания:")
	var list string
